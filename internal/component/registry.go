	...
	return []Component{
		{
			...
			Files: []TemplateFile{
				...
				{Source: "web/readiness.go.tmpl", Target: "{{ if eq .Recipe.Layout.Style \"minimal\" }}internal/app/readiness.go{{ else }}internal/server/handler/readiness.go{{ end }}"},
		...
		{
			ID:          IDTaskSchedulerNone,
			Category:    CategoryTaskScheduler,
			Name:        "No task scheduler",
			Description: "Project without a scheduled task executor.",
			Conflicts:   []string{IDTaskSchedulerGocron},
		},
		{
			ID:          IDTaskSchedulerGocron,
			Category:    CategoryTaskScheduler,
			Name:        "gocron task scheduler",
			Description: "Scheduled task executor backed by gocron.",
			Requires:    []string{IDProjectWeb},
			Conflicts:   []string{IDTaskSchedulerNone},
			Files: []TemplateFile{
				{Source: "web/scheduler.go.tmpl", Target: "internal/scheduler/scheduler.go"},
				{Source: "web/example_cleanup.go.tmpl", Target: "internal/scheduler/tasks/example_cleanup.go"},
			},
			GoModules: []GoModule{{Path: "github.com/go-co-op/gocron/v2", Version: "v2.21.0"}},
		},
		{
			ID:          IDLoggingSlog,
